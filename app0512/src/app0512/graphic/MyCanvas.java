...
public class MyCanvas extends Canvas{
	...
	/*캔버스에 그려질 그림을 개발자가 주도하여,
	원하는 그림을 그려보자!!
	paint 메서드에 대하여...
	특징 : 개발자가 paint메서드를 호출해서는 안된다!!!
			왜?? 자바의 모든 컴포넌트는 스스로를 그린다.. 즉 paint메서드는 시스템이
			그래픽의 랜더링 시킬 필요에 의해 호출하게 됨..
			...
			sun에서는 그래픽이 화면에 반영되는 처리는 시스템이 하기 때문에 paint()메서드 호출을
			원칙적으로 막아놓았다. 따라서 개발자는 시스템에게 paint()가 호출될 수 있도록 간적적으로
			요청을 하는 방식으로 그래픽처리를 할 수 있다.. 이때 호출해야 할 메서드가 바로 repaint().
			repaint() 그래픽의 갱신을 요청하게 되고, 이때 이 요청이 들어오면 update()가 호출되어
			화면을 지우며, 화면을 지우고나서 깨끗해지면 paint() 호출
