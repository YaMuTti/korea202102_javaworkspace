		...
		try{
			...
			if(con!=null){
				...
				while(rs.next()){ //rs.next(); 반환값이 true인 동안 반복문 수행
					...
					//현재 커서가 기리키는 row의 컬럼들을 하나씩 접근해보자!!
					/*
					...
					*/
					...
					String user_id=rs.getString(2); //첫번째 컬럼
					String password=rs.getString(3); //첫번째 컬럼
					String name=rs.getString(4); //첫번째 컬럼
					String regdate=rs.getString(5); //첫번째 컬럼
			...
			//DriverManaver.getConnection()으로 접속을 시도하다가 에러가 나도 SQLException
