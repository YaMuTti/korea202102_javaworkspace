...
//네트워크를 제어할 수 있도록, 추상화시켜놓은 api를 지칭, 거의 모든 응용프로그램 마다 지원
public class BasicServer {
	ServerSocket server; //소켓 서버 구착을 위한 서버 객체 (접속을 받아들이는 소켓)
	
	//네트워크 프로그램시 항상 염두해두어야할 점은 네트워크 프로그램의 원리가 전화의 원리와 상당히 흡사하다는 점.
	...
	public BasicServer() {
		try {
			...
			//접속자가 발견되면, 이 시점부터는 접속자인 클라이언트와 대화를 나눌수 있으므로, 대화를 주고받는 전화기에 해당하는 소켓을 반환받는다!!
			
			InputStream is = socket.getInputStream();
			int data=-1;
			while(true) {
				//아래의 read() 메서드는 모든 입력스트림의 read() 메서드는 실행부로 하여금, 데이터가
				//들어올때까지는 대기상태에 빠지게 한다.. 즉 기다린다..
				data=is.read(); //소켓과 연결된 입력스트림으로부터, 클라이언트의 메시지 중 1byte를 읽어들임
				//즉 말이 네트워크 프로그램, 즉 우리가 연결한 대상이 단지 네트워크일 뿐이며, 실제적인 프로그램은 스트림 제어다
